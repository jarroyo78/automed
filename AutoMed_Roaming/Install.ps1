...
$requiredFiles = @(
    @{ File = "Medicus.Xamarin.UWP_1.0.0.0_x64.cer"; Url = "$script:RepoBase/Medicus.Xamarin.UWP_1.0.0.0_x64.cer"; MinSize = 1KB },
    @{ File = "Microsoft.NET.CoreRuntime.2.2.appx"; Url = "$script:RepoBase/Microsoft.NET.CoreRuntime.2.2.appx"; MinSize = 100KB },
    @{ File = "Microsoft.NET.CoreFramework.Debug.2.2 - Copy.appx"; Url = "$script:RepoBase/Microsoft.NET.CoreFramework.Debug.2.2%20-%20Copy.appx"; MinSize = 100KB },
    @{ File = "Microsoft.VCLibs.x64.14.00.appx"; Url = "$script:RepoBase/Microsoft.VCLibs.x64.14.00.appx"; MinSize = 500KB },
    @{ File = "Microsoft.VCLibs.x64.14.00.Desktop.appx"; Url = "$script:RepoBase/Microsoft.VCLibs.x64.14.00.Desktop.appx"; MinSize = 5MB },
    @{ File = "Medicus.Xamarin.UWP_1.0.411.0_x64.appx"; Url = "$script:RepoBase/Medicus.Xamarin.UWP_1.0.411.0_x64.appx"; MinSize = 90MB }
)
...
foreach ($dep in $deps) {
    ...
    try {
        ...
        # Provision for all future users (roaming profile support)
        Add-AppxProvisionedPackage -Online -PackagePath $dep -SkipLicense -ErrorAction SilentlyContinue
        Write-Ok "$leaf provisioned for all users."
    } catch {
...
foreach ($vclib in $vclibsFiles) {
    if (Test-Path $vclib) {
        ...
        try {
            ...
            Add-AppxProvisionedPackage -Online -PackagePath $vclib -SkipLicense -ErrorAction SilentlyContinue
            Write-Ok "$name provisioned for all users."
        } catch {
            if ($_.Exception.Message -match "0x80073D06|already installed") {
...
try {
    ...
    # Provision for all future users (roaming profile support)
    Write-Host "    Provisioning Medicus for all users..." -ForegroundColor Gray
    Add-AppxProvisionedPackage -Online -PackagePath $mainApp -SkipLicense -ErrorAction SilentlyContinue
    Write-Ok "Medicus provisioned for all future users."
} catch {
...
# 11b. Register for all existing profiles (roaming profile support)
Write-Step "Registering Medicus for all existing user profiles..."
try {
    $allUsersPackages = Get-AppxPackage -AllUsers | Where-Object { $_.Name -like '*Medicus*' -or $_.Name -like '*medicus*' }
    ...
    Write-Ok "Medicus registered for all existing profiles."
} catch {
    Write-Warn "Could not register for all profiles: $_"
}
...
try {
    ...
    if ($appPackage) {
        ...
        # Extract the app icon to a public location so all users can read it
        ...
        # Find the best icon source (prefer PNG with highest resolution)
        ...
        if (-not $srcPng) {
            $srcPng = Get-ChildItem (Join-Path $appPackage.InstallLocation "Assets") -Filter "*.png" |
        ...
        if ($srcPng -and (Test-Path $srcPng)) {
            ...
            Write-Ok "Icon extracted to $iconPath"
        }
        ...
        if (Test-Path $iconPath) {
            ...
            Write-Warn "Icon file not found, shortcut will use default icon."
        }
        $shortcut.Save()
